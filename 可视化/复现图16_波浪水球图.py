# -*- coding: utf-8 -*-
"""图16 波浪水球图：严格读取 Excel 数据，完整显示中文标题、标签、图例与脚注。"""
...
def create_chart():
    value = float(read_rows(["B"], 4, 4)[0][0])
    fig, ax = canvas((9.4, 6.5))
    tx(fig, .07, .90, "本科及以上学历员占比", color=WHITE, fontsize=24, bold=True)
    tx(fig, .07, .835, "6月30日最新统计数据本科及以上员工占比65%", color=WHITE, fontsize=16)
    tx(fig, .07, .075, "*注：数据来源于公司人力资源系统", color=WHITE, fontsize=9)
    ax.set_position([.28, .16, .44, .62]); ax.set_aspect("equal"); ax.axis("off")
    ax.set_xlim(-1.08, 1.08); ax.set_ylim(-1.08, 1.08)
    boundary = Circle((0, 0), 1, facecolor="white", edgecolor=BRIGHT_BLUE, lw=2.3)
    ax.add_patch(boundary)
    x = np.linspace(-1.08, 1.08, 1000)
    y0 = -1 + 2 * value
    wave = y0 + .075 * np.sin(3.1*np.pi*x + .75)
    water = ax.fill_between(x, -1.1, wave, color=BLUE, zorder=2)
    water.set_clip_path(boundary)
    line, = ax.plot(x, wave, color="#2BB1FF", lw=2, zorder=3)
    line.set_clip_path(boundary)
    tx(ax, 0, -.02, f"{value:.0%}", color=WHITE, fontsize=34, bold=True,
