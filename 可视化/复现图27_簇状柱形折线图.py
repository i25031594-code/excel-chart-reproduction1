# -*- coding: utf-8 -*-
"""图27 簇状柱形折线图：严格读取 Excel 数据，完整显示中文标题、标签、图例与脚注。"""
...
SHEET_NAME = "27 簇状柱形折线图"
OUTPUT_FILE = "图27_簇状柱形折线图.png"
...
def create_chart():
    ...
    labels=[str(r[0]) for r in rows]; y22=np.array([float(r[1]) for r in rows]); y21=np.array([float(r[2]) for r in rows]); yoy=np.array([float(r[3]) for r in rows])
    fig,ax=canvas()
    header(fig,"2022","上半年各月商品销量同比去年情况","2022年相比于2021年销量都有提升，半年整体提升17%",[("2022","本期"),("2021","同期"),("17%","半年整体提升")])
    footer(fig,source="注：数据来源于公司销售系统，统计日期截至2022.06.30")
    ax.set_position([.10,.16,.80,.54]); x=np.arange(len(labels)); w=.27
    b1=ax.bar(x-w/2,y22,w,color=BLUE,label="2022销量"); b2=ax.bar(x+w/2,y21,w,color=PINK,label="2021销量")
    for bars in (b1,b2):
        for b in bars: tx(ax,b.get_x()+b.get_width()/2,b.get_height()+48,str(int(b.get_height())),color=WHITE,fontsize=7.5,ha="center")
    ax2=ax.twinx(); ax2.plot(x,yoy,color="#FFD000",marker="o",ms=5,lw=2,label="同比去年")
    for xx,v in zip(x,yoy): tx(ax2,xx,v+.012,f"{v:.0%}",color=WHITE,fontsize=8,ha="center")
    ax.set_xticks(x,labels); style_category_axis(ax); ax.set_ylim(0,4300); ax2.set_ylim(0,.30)
    ax.tick_params(axis="y",left=False,labelleft=False); ax2.tick_params(right=False,labelright=False)
    ax.spines[:].set_visible(False); ax2.spines[:].set_visible(False); ax.axhline(0,color=GRID); ax.set_facecolor(BG); ax2.set_facecolor("none")
    h1,l1=ax.get_legend_handles_labels(); h2,l2=ax2.get_legend_handles_labels()
    leg=ax.legend(h1+h2,l1+l2,loc="upper center",ncol=3,frameon=False,bbox_to_anchor=(.5,1.04),labelcolor=WHITE,fontsize=7.5)
