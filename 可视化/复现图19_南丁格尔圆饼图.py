# -*- coding: utf-8 -*-
"""图19 南丁格尔圆饼图：严格读取 Excel 数据，完整显示中文标题、标签、图例与脚注。"""
...
SHEET_NAME = "19 南丁格尔圆饼图"
OUTPUT_FILE = "图19_南丁格尔圆饼图.png"
...
def create_chart():
    ...
    colors=[PINK,YELLOW,CYAN,BLUE,LIGHT_BLUE,"#7237A8"]
    ...
    n=len(values); theta=np.linspace(0,2*np.pi,n,endpoint=False); radii=np.sqrt(values/values.max())
    ...
    for a,r,lab,val,col in zip(theta+np.pi/n,radii,labels,values,colors):
        tx(ax,a,r+.14,f"{lab}\n{val:.1%}",color=col,fontsize=8.5,ha="center",va="center")
    finish(fig)
