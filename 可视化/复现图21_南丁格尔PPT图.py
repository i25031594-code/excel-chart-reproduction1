# -*- coding: utf-8 -*-
"""图21 南丁格尔PPT图：严格读取 Excel 数据，完整显示中文标题、标签、图例与脚注。"""
...
SHEET_NAME = "20 南丁格尔（PPT）"
OUTPUT_FILE = "图21_南丁格尔PPT图.png"
...
def create_chart():
    rows=read_rows(["B","C"],3,8)
    labels=[str(r[0]).strip() for r in rows]; values=np.array([float(r[1]) for r in rows])
    colors=[PINK,YELLOW,CYAN,LIGHT_BLUE,BLUE,"#7237A8"]
    fig=plt.figure(figsize=(10,6.2),facecolor=BG)
    header(fig,"2021","2021年各部门人数分布","公司总人数1664，销售部人数最多451，占比29.2%",
           [("1664","公司总人数"),("451","销售部人数"),("29.2%","销售部占比")])
    footer(fig,date="2022.01.01")
    ax=fig.add_axes([.20,.12,.60,.66],projection="polar",facecolor=BG)
    n=len(values); theta=np.linspace(0,2*np.pi,n,endpoint=False); radii=.34+.68*values/values.max()
    ax.bar(theta,radii,width=2*np.pi/n,bottom=0,color=colors,edgecolor=BG,linewidth=1.2,align="edge")
    ...
    for a,r,lab,val,col in zip(theta+np.pi/n,radii,labels,values,colors):
        tx(ax,a,r+.13,f"{lab}  {val:.1%}",color=col,fontsize=8.3,ha="center")
    finish(fig)
